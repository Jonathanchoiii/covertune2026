...
## CoverTune Product Decisions

- The selected visual truth is `design-reference.png` (the first Product Design ideation result).
- Preserve its fog-gray gallery atmosphere, floating asymmetric album cards, warm-white tactile surfaces, black grotesk hierarchy, and acid-yellow action accent.
- The core demo state is the result for `JON`; the three locally generated cover assets must remain the visual reference for J, O, and N.
- The product must accept 1–10 Latin letters. Layout modes are deliberate: one hero card for 1 letter, a floating constellation for 2–3, a two-column gallery for 4–6, and a compact three-column index for 7–10.
- Open music records from Internet Archive Netlabels are real source-backed candidates and must display their item and Creative Commons license links. Generated demo artwork must never be presented as a real release.
- Name changes update results automatically after a short debounce; Enter is optional.
- The command dock intentionally has no copy action. Saving uses the mobile system
  share sheet when available and a PNG download on desktop.
- Letter matching is visual-first. A release title or artist initial is not
  evidence that its artwork represents a letter.
- `CHLOE` is the first fully reviewed real-release set: CRO `C`, Ayumi Hamasaki
  `H`, KinKi Kids `L album`, De Staat `O`, and Ecco2k `E`.
- `effect-reference-chloe.png` is the product reference for visual letter
  matching: the artwork itself must contain the letter cue.
- The reviewed visual catalog covers all A–Z letters with at least three
  candidates per letter. “换一组” rotates through those per-letter pools, and
  repeated letters should prefer different candidates when possible.
